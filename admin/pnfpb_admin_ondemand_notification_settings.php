			...
			if (($apiaccesskey != '' && $apiaccesskey != false)  || get_option('pnfpb_httpv1_push') === '1' || (get_option( 'pnfpb_onesignal_push' ) === '1') || (get_option( 'pnfpb_progressier_push' ) === '1') || (get_option( 'pnfpb_webtoapp_push' ) === '1')) {
					...
					if (get_option('pnfpb_webtoapp_push') === '1') {
						...
						if (isset($_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable']) && $_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable'] === '1') {
							...
							$action_scheduler_status = as_schedule_single_action( time(), 'PNFPB_webtoapp_schedule_push_notification_hook', array(0, stripslashes(strip_tags($_POST['pnfpb_ic_on_demand_push_title'])), $activity_content_push, $postlink, $imageurl));	
					...
					if (get_option('pnfpb_progressier_push') === '1') {
						...
						if (isset($_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable']) && $_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable'] === '1') {
							...
							$action_scheduler_status = as_schedule_single_action( time(), 'PNFPB_progressier_schedule_push_notification_hook', array(0, stripslashes(strip_tags($_POST['pnfpb_ic_on_demand_push_title'])), $activity_content_push, $postlink, $imageurl));
						...
						if (get_option('pnfpb_onesignal_push') === '1') {
							...
							if (isset($_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable']) && $_POST['pnfpb_ic_fcm_ondemand_schedule_now_enable'] === '1') {
								...
								$action_scheduler_status = as_schedule_single_action( time(), 'PNFPB_onesignal_schedule_push_notification_hook', array(0, stripslashes(strip_tags($_POST['pnfpb_ic_on_demand_push_title'])), $activity_content_push, $postlink, $imageurl));
  						...
  						<div class="pnfpb_column">
    						<div class="pnfpb_card">
								...
								<label class="pnfpb_ic_push_settings_table_label_checkbox" for="pnfpb_ic_fcm_ondemand_schedule_now_enable">
									<?php echo __("Send notification in action scheduler asynchronous background mode to avoid server overload.",'PNFPB_TD');?>
								</label>
							</div>
						</div>
					</div>
